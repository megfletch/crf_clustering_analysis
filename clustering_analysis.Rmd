...
attributes_long <- melt(case_clustering, id = c("case_study", "dimension"))
...
# Prepare data for dendrogram
# Scale the data
eco_heat_scaled <- eco_heat_clustering
eco_heat_scaled[, c(2:14)] <- scale(eco_heat_scaled[, 2:14])
```

```{r}
# Select the appropriate columns
eco_long <- melt(eco_heat_scaled, id = c("case_study"))
...
# Prepare data for dendrogram
# Scale the data
socio_heat_scaled <- socio_heat_clustering
socio_heat_scaled[, c(2:16)] <- scale(socio_heat_scaled[, 2:16])
```

```{r}
# Select the appropriate columns
socio_long <- melt(socio_heat_scaled, id = c("case_study"))
...
# Prepare data for dendrogram
# Scale the data
gov_heat_scaled <- gov_heat_clustering
gov_heat_scaled[, c(2:11)] <- scale(gov_heat_scaled[, 2:11])
```

```{r}
# Select the appropriate columns
gov_long <- melt(gov_heat_scaled, id = c("case_study"))
