...
attributes_scaled[, c(2:39)] <- scale(attributes_scaled[, 2:39])

# Run clustering
attributes_matrix <- as.matrix(attributes_scaled[, -c(1)])
rownames(attributes_matrix) <- attributes_scaled$case_study
attributes_dendro <- as.dendrogram(hclust(d = dist(x = attributes_matrix)))


# PLot the dendrogram
dendrogram_plot <- ggdendrogram(data = attributes_dendro, rotate = TRUE)
