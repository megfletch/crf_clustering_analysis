...
```{r}
attributes_merged <- rbind(aguion_barnacle, burden_sardine, dickey_collas_eur, eurich_clam, free_crab, friedman_fiji, kleisner_jfi, lau_png, mason_iceland, mills_lobster, pecl_lobster, tokunaga_lobster, westfall_hms, zhao_moorea)
...
attributes_heatmap_plot <- ggplot(data = attributes_long, 
  ...
  labs(title = "Heatmap of Attribute Scores by Case study", x = "Case Studies", y = "Attributes") +
...
info_clustering <- attributes_merged_clean %>% 
  select(case_study, attribute, numeric_info) %>% 
  # Use `pivot_wider` function to decrease the number of rows and increase the number of columns
  # Also do values_fill = 0 to convert NA values to 0
  pivot_wider(names_from = attribute, values_from = numeric_info, values_fill = 0)
```
