...
aguion_table <- aguion_barnacle %>% 
  ...
  pivot_wider(names_from = c(importance), values_from = attribute) %>% 
  rename(" " = score_value) 
...
```{r}
aguion_matrix <- aguion_barnacle %>% 
  select("attribute", "importance", "score_value") %>% 
  pivot_wider(names_from = score_value, values_from = attribute) 

ggplot(aguion_barnacle, aes(x = importance, y = score_value)) +
  geom_tile(fill = "lightblue") +
  labs(title = "Aguion Attribute Matrix", x = "Importance", y = "Score") +
  geom_text(aes(label = attribute), 
            color="navy",
            size = 3,
            position = position_jitter(width=0, height=0.35)) +
